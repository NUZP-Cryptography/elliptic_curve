...
def multi_signature():
	...
	r = (sum_of_points_R['x']) % sigma
	print(f'\nПісля чого формується число r , r = ... = {sum_of_points_R["x"]} mod {sigma} = {r}')
	
	print(f'\nПотім кожний користувач за допомогою свого особистого ключа d_i, значення k_i, хеш-образу h_i обчислює свою частину підпису ...:')

	s = 0
	for i in range(t):
		_s = (array_of_k[i] - array_of_d[i] * h[i] * r) % n
		print(f's_i = k_i - d_i·h_i·r mod n = {array_of_k[i]} - {array_of_d[i]}·{h[i]}·{r} mod {r} = {_s}')
		s += _s
...
def check_multi_signature(data):
	...
	sum_of_points_Q = multiplication_point(h[0]-1, array_of_points_Q[0], a, b, p)
	for i in range(t - 1):
		sum_of_points_Q = addition_of_points(multiplication_point(h[i + 1]-1, array_of_points_Q[i + 1], a, b, p), sum_of_points_Q, a, b, p)

	print(
		f'\nПеревірка підпису <r, s> (<{r}, {s}>) під набором електронних документів [M_1, M_2, ..., M_i] здійснюється за допомогою додаткової точки еліптичної кривої  Q = ... , Q = {sum_of_points_Q["x"], sum_of_points_Q["y"]}')
	...
	print(
		f'Після чого обчислюються хеш-образ документу Н(М), відповідне десяткове число h та формується число r ̃ = h⋅xR ̃ mod δ = {R["x"]} mod {sigma} = {r}.')
	print(
